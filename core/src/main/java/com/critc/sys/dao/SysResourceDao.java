...
public class SysResourceDao extends BaseDao<SysResource, SysResource> {
    ...
    public SysResource get(int id) {
        String sql = "select t.id,t.name,t.code,t.parent_id,t.url,t.target,t.iconimg,t.display_order,t.type,t.description,t.resource_level,(select name from t_sys_resource where id=t.parent_id) parent_name from t_sys_resource t where id=?";
        return get(sql, id);
    }
    ...
    public List<SysResource> listByType(int type) {
        String sql = "select t.id,t.name,t.code,t.parent_id,t.url,t.target,t.iconimg,t.display_order,t.type,t.description,t.resource_level,(select count(*) from t_sys_resource where parent_id=t.id) cnt from t_sys_resource t where type =? order by parent_id, display_order";
    ...
    public List<SysResource> listByParentId(int parentId,int type) {
        String sql = "select t.id,t.name,t.code,t.parent_id,t.url,t.target,t.iconimg,t.display_order,t.type,t.description,(select count(*) from t_sys_resource where parent_id=t.id) cnt from t_sys_resource t where parent_id=? and type=? order by display_order";
