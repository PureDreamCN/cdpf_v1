...
<div class="page-bar">
													...
													<tr role="row">
														...
														<td>
														     <c:if test="${critc:isP('SysDicUpdate')}">
												                 <a class="edit" href="toUpdate.htm?id=${sysDic.id }&backUrl=${backUrl}"> 修改 </a>
											                 </c:if> 
											             </td>
											             <td>
											                 <c:if test="${critc:isP('SysDicDelete')}">
												                 <a class="delete" href="javascript:delSysDic(${sysDic.id});"> 删除 </a>
											                </c:if>
											             </td>
													</tr>
												</c:if>
 ...
 <script type="text/javascript"> 
     ...
     jQuery.validator.addMethod('checkDicExist', function(value, element,param){
    	  ...
    	  if(param[0] == 'code'){
    		  params = {"param":param[0],"prev":prev, "value":value,"category":$("#categoryParent").val()};
    	  }else{
    	...
    	//1、判断字典类别是否改变
    	 ...
    	 $("table tr").each(function(){
    		  ...
    		  if($(this).hasClass("newRow")){
    			   ...
    			   if($(this).find(":input:first").attr("name")!="id"){
    				   if($(this).children("td:eq(0)").find("input").hasClass("valid")){
