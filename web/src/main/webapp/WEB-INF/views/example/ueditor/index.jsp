...
<head>
    ...
    <!--  <script type="text/javascript" charset="utf-8">
        window.UEDITOR_HOME_URL = "${staticServer}/assets/example/ueditor"; //UEDITOR_HOME_URL、config、all这三个顺序不能改变
    </script> -->
...
&lt;/script>
		...
		return;
						      ...
						      <p> 4、图片上传的问题 </p>
						      <p>主要涉及到的文件有： config.json, ueditor.all.js,ueditor.config.js,image.js,controller.jsp
						       4.1、在项目中添加ueditor的maven 依赖</p>
                               <pre>
				   ...
				   &lt;/dependency>
						      ...
						      <p>4.2、修改config.json 上传图片配置项 中的两个属性的路径（根据项目）
						        "imageUrlPrefix": "http://localhost:8080/web/", /* 图片访问路径前缀 */ 
                                "imagePathFormat": "/upload/ueditor/{yyyy}{mm}{dd}/{time}{rand:6}", /* 上传保存路径,可以自定义保存路径和文件名格式 */  </p>   
                                                                               配置不成功的现象：在浏览器会报不支持图片上传的错，并且单个图片上传按钮是灰色的<br/><br/> 
                             <p> 4.3、点击单个图片上传响应很慢的解决方法 
							 ...
							 <p> 4.4、点击上传多张图片响应很慢的解决方法 
							      响应很慢的原因： 与单张图片类似 
							      解决方案： 修改dialogs/image/imgage.js 中370 或者查找accept: {
							      将其中的mimeTypes 属性改为 'image/jpeg,image/jpg,image/png,image/gif' </p>

                             <p> 4.5、在线图片管理不能显示已上传图片的解决方法 </p> 
							 <p>4.5.1、修改controller.jsp中代码  
								具体详查controller.jsp 文件  </p>
                               
                                <p>  4.5.2、修改config.json文件  
								        在config.json中找到 “在列出指定目录下的图片” 
								        修改 "imageManagerUrlPrefix": "http://localhost:8080/web"和 
								       "imageManagerListPath": "/upload/ueditor",属性（根据具体的项目而定）
								   </p>
