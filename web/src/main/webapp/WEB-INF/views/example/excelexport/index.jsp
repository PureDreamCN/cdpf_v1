		...
		<div class="portlet-body">
			<div class="table-responsive">
				<table class="searchTable">
					<tr>
						<td>学号：</td>
                    <td><input type="text" id="txtStudentno" class="form-control input-middle"
                               placeholder=""
                               value="${exampleStudentSearchVO.no }"></td>
                    <td>姓名：</td>
                    <td><input type="text" id="txtStudentname" class="form-control input-middle"
                               placeholder=""
                               value="${exampleStudentSearchVO.name }"></td>
					<td>学院：</td>
                    <td><form:select path="exampleStudentSearchVO.collegeNo" class="form-control"
                                     id="cmbCollegeNo">
                        <form:option value="" label="--请选择学院--"/>
                        <form:options items="${listCollege}" itemValue="value" itemLabel="content"/>
                    </form:select>
                    </td>
                    <td>专业：</td>
                    <td><form:select path="exampleStudentSearchVO.majorNo" class="form-control"
                                     id="cmbMajorNo">
                        <form:option value="" label="--请选择专业--"/>
                        <form:options items="${listMajor}" itemValue="value" itemLabel="content"/>
                    </form:select>
                    </td>
						...
						<td>
							...
							</button> <c:if test="${critc:isP('ExampleExcelExportExport')}">
								<button type="button" class="btn btn-success" id="btnExport" onclick="exportList()">
	...
	<div class="row">
		<div class="col-md-12">
			<table id="studentTable"
				...
				<thead>
					<tr>
						<th width="40px"><input name="checkAllChange" type="checkbox" id="checkAllChange"/><span></span> #</th>
				...
				<tbody>
					<c:forEach items="${list }" var="student" varStatus="st">
						<tr>
							<td><input name="stuno" type="checkbox" value="${student.no }">&nbsp;${st.index+1 }</td>
							<td>${student.no }</td>
							<td>${student.name }</td>
	...
	type="text/javascript"></script> <script type="text/javascript">
		...
		var searchMajor = function(){					
		    ...
		    $.getJSON(url, function (data) {
		    	...
		    	$("#cmbMajorNo").append("<option value=''>&nbsp;" + '--请选择专业--' + "</option>");	
		        $.each(data, function (i, item) {
		        	$("#cmbMajorNo").append("<option value='" + item.value + "'>&nbsp;" + item.content + "</option>");
		        });
		    });
		}
